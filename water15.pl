...
$target_prof = 11;	### TARGET PROFICIENCY OF SPELL REALM
$current_prof = 10;
...
$max_hp = 92;		# Max HP of character
$max_mp = 12;		# HP at which character must heal
...
while ($fumble_mode) {
	...
	# Check if mob died from blister
	if ($input =~ /You gained/) {
		if ($debug) {
			print ">>> $mob killed\n";
		}
	...
	if ($input =~ /(\d+) H (\d+) M/) {
		if ($2 < 3) {
			if ($debug) {
				print ">>> NO MORE MP ($2) FOR BLISTER\n";
			}
			$fumble_mode = 0;
			$no_mp = 1;
		}
